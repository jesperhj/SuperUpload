  ...
  class Request
    ...
    def parse(data)
      if @parser.finished?  # Header finished, can only be some more body
        ...
        #######################################################################
        # Get info for non IE browsers
        #######################################################################
        form_id = nil        
        form_id_match = data.to_s.match(/form-data; name="file-(.*)";/)
        filename_match = data.to_s.match(/filename="(.*)"/)
        
        if !form_id_match.nil? && !filename_match.nil?
          form_id  = form_id_match[1]
          filename = filename_match[1].match(/(.*\\){0,1}(.*)$/)[2]
        end
        #######################################################################
        ...
        #######################################################################
        # Modified call to method with 2 params
        #######################################################################
        move_body_to_tempfile form_id, filename if @parser.finished? && content_length > MAX_BODY
        #######################################################################
      end
    ...
    #######################################################################
    # method for accessing the tempfiles path, nil of not set
    #######################################################################
    def tempfile_path
      @env[RACK_INPUT].path
    end
    ...
    private
      def move_body_to_tempfile(form_id = nil, filename = nil)
        current_body = @body
        current_body.rewind
        @body = Tempfile.new(BODY_TMPFILE)
        
        #######################################################################
        # Store file with info for non IE
        #######################################################################
        info = {
          :size     => content_length,
          :path     => @body.path,
        ...
        if !form_id.nil? && !filename.nil?
          puts "saving file for ff and chrome"
          file_info = File.new("/tmp/uploader_file-#{form_id}", 'w')
          file_info.puts info.to_json
          file_info.close
        end
        #######################################################################
